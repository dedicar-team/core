...
import { costumerController } from '_app/_app.module'
...
export const costumerProcedures = trpc.router<TTrpcContext>()
    .mutation('Costumer.login', {
        input: costumerController.zod.login,
        async resolve({ input, ctx }) {
            if (ctx.req.session.personUuid) throw new TRPCError({ code: 'FORBIDDEN', message: 'already logged at this client.' })
            const data = await costumerController.login(input)
            const [refinedData] = await Promise.all([
                costumerController.refine.login(data),
                (async () => createSession(ctx, { personUuid: data.Profile.Person.id }))()
            ])
            return refinedData
        },
    })
    .mutation('Costumer.register', {
        input: costumerController.zod.register,
        resolve: async ({ ctx, input }) => {
            try {
                const costumer = await costumerController.register(input)
                const [refinedCostumer] = await Promise.all([
                    costumerController.refine.register(costumer),
                    (async () => createSession(ctx, { personUuid: costumer.Profile.Person.id, }))()
                ])
                return refinedCostumer
            } catch (e) {
                console.log(e)
            }
        }
